...
describe("Config validation", () => {
  // Store original env vars and restore a baseline before each test
  beforeEach(() => {
    vi.resetModules();
    // Restore baseline env vars
  ...
  it("passes when all required env vars are present", async () => {
    const { env } = await import("../lib/config.js");
    expect(env.DISCORD_TOKEN).toBe("test-token");
    expect(env.DISCORD_CLIENT_ID).toBe("test-client-id");
...
describe("Graceful shutdown", () => {
  it("prisma instance has $disconnect method", async () => {
    const { PrismaClient } = await import("@prisma/client");
